...
-e 'torctl-log' | python parse-shadow.py -\n
"""
...
def single_parse(args):
    ...
    for line in args.datasource:
        try:
            ...
            if parsetimestamp(parts[2]) < MINVALIDSECS: continue
            # check for our log line types
            if 'shadow-heartbeat' in parts[6]: shadow_helper(sdata, parts)
            elif 'torctl-log' in parts[6]: tor_helper(tdata, parts)
            elif 'fg-download-complete' in parts[6]: filetransfer_helper(fdata, parts)
        except: continue
    if 'xzproc' in args: args.xzproc.wait()
    dump(args, sdata, SHADOWJSON)
    dump(args, tdata, TORJSON)
    dump(args, fdata, FILETRANSFERJSON)
...
def mutli_parse(args):
    ...
    for i in xrange(NUMFILTERWORKERS): pool.append(Process(target=filterworker_main, args=(filterQ, shadowQ, torQ, filetransferQ)))
    ...
    for Q in [filetransferQ, torQ, shadowQ]: Q.put('STOP')
    for Q in [filetransferQ, torQ, shadowQ]: Q.join()

def filterworker_main(filterQ, shadowQ, torQ, filetransferQ):
    signal(SIGINT, SIG_IGN) # ignore interrupts
    shadowbatch, torbatch, filetransferbatch = [], [], []
    for linebatch in iter(filterQ.get, 'STOP'):
        for line in linebatch:
            try: # skip malformed lines
                ...
                # dont include bootstrapping period
                if parsetimestamp(parts[2]) < MINVALIDSECS: raise
                # check for our log line types
                if 'shadow-heartbeat' in parts[6]: shadowbatch.append(parts)
                elif 'torctl-log' in parts[6]: torbatch.append(parts)
                elif 'fg-download-complete' in parts[6]: filetransferbatch.append(parts)
            except: pass
...
def dump(args, data, filename): # inline compression
...
def dump2(args, data, filename): # no compression
    if not os.path.exists(args.prefix): os.makedirs(args.prefix)
