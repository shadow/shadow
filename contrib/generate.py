...
import sys, os, argparse, pygeoip, random, socket

def main():
    parser = argparse.ArgumentParser(description='Utility to generate Tor-style topologies for the scallion plugin to the shadow simulator. You should run this generator in the directory from which you plan to run shadow, and give correct paths to the required files.', formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    # network configuration
    parser.add_argument('--nrelays', action="store", type=int, dest="nrelays",
          help="number of total relays for the generated topology", metavar='N', default=40)
    parser.add_argument('--fexit', action="store", type=float, dest="exitfrac",
          help="fraction of relays that are exits", metavar='F', default=0.4)
    parser.add_argument('--nclients', action="store", type=int, dest="nclients",
          help="number of total clients for the generated topology", metavar='N', default=2640)
    parser.add_argument('--fbulk', action="store", type=float, dest="fbulk",
          help="fraction of bulk downloading clients (remaining are web clients)", metavar='F', default=0.05)
    parser.add_argument('--nservers', action="store", type=int, dest="nfs",
          help="number of fileservers", metavar='N', default=200)

    # torrc configuration files
    parser.add_argument('--exit', action="store", type=str, dest="exitrc",
          help="path to a torrc for exit Tor relays", metavar='PATH', default="config/exit.torrc")
    parser.add_argument('--relay', action="store", type=str, dest="relayrc",
          help="path to a torrc for nonexit Tor relays", metavar='PATH', default="config/relay.torrc")
    parser.add_argument('--client', action="store", type=str, dest="clientrc",
          help="path to a torrc for Tor clients", metavar='PATH', default="config/client.torrc")
    parser.add_argument('--authority', action="store", type=str, dest="authrc",
          help="path to a torrc for the Tor directory authority", metavar='PATH', default="config/authority.torrc")
    
#    parser.add_argument('--authority-keys', action="store", type=str, dest="keys",
#          help="path to Tor directory authority keys directory (This can be created with the command\n\"tor --list-fingerprint --DataDirectory . -f authkeys.torrc\"\nwhere authkeys.torrc contains\n\"DirServer test 127.0.0.1:5000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000\nORPort 5000\"", metavar='PATH/TO/AUTHORITY/KEYS', default="config/keys")

    # other files needed for tor
    parser.add_argument('--consensus', action="store", type=str, dest="consensus",
          help="path to a Tor consensus file", metavar='PATH', default="config/consensus.txt")
    
    parser.add_argument('--geoip', action="store", type=str, dest="geoip",
          help="path to the Tor geoip file", metavar='PATH', default="resources/geoip")

    # files needed for shadow
    parser.add_argument('--resources', action="store", type=str, dest="cdfs",
          help="path to shadow resources like the CDF package", metavar='PATH', default="resources/")
    
    parser.add_argument('--data', action="store", type=str, dest="data",
          help="path to the output directory for data produced in the simulation", metavar='PATH', default="data/")

    parser.add_argument('--topology', action="store", type=str, dest="run",
          help="path to the output directory for files generated by this script needed for running scallion", metavar='PATH', default="topology/")

    parser.add_argument('--geoipcity', action="store", type=str, dest="geoipcity",
          help="path to the Maxmind geoip city database needed for the pygeoip python module", metavar='PATH', default="/usr/local/share/GeoIP/GeoIPCity.dat")
    
    parser.add_argument('--dsim', action="store", type=str, dest="dsimout",
          help="filename of the simulation dsim file generated by this script (will be placed in \'topology\' directory)", metavar='FILENAME', default="scallion.dsim")

    parser.add_argument('--no-validate', action="store_true", dest="novalidate",
          help="do not validate that paths exist", default=False)
    ...
    if args.data[len(args.data)-1:len(args.data)] != '/': args.data += '/'
    if args.run[len(args.run)-1:len(args.run)] != '/': args.run += '/'
    if args.cdfs[len(args.cdfs)-1:len(args.cdfs)] != '/': args.cdfs += '/'

    if not os.path.exists(args.run):
        os.makedirs(args.run)

    if not args.novalidate:
        validate_path(args.exitrc)
        validate_path(args.relayrc)
        validate_path(args.clientrc)
        validate_path(args.authrc)
        validate_path(args.consensus)
        validate_path(args.geoip)
#        validate_path(args.data)
        validate_path(args.cdfs)
        validate_path(args.geoipcity)
        validate_path(args.run)
        validate_path(args.cdfs + "/http_think_times.cdf")
        validate_path(args.cdfs + "/bandwidth_up_KBps.cdf")
        validate_path(args.cdfs + "/cpu_speed_Bps.cdf")

    print >> sys.stderr, "\033[91m" + "!I: please remember to place the following files in \'" + args.run + "\': " + ", ".join(["libpreload.so", "libplugin-fileserver.so", "libscallion.so", "libscallion_preload.so", "shadow"]) + "\033[0m"
...
def validate_path(path):
    if not os.path.exists(path): 
        print >> sys.stderr, "!E: path does not exist: " + path
        exit(-1)
...
class ScallionExperiment():
    def __init__(self, args):
        ...
        self.outfile = open(args.run + args.dsimout, 'w')
        self.relay_choices = []
...
"?DSIM3\n\
on(time 0:01){\n\
\tfs_plugin = load_plugin(\"./libplugin-fileserver.so\");\n\
\tscallion_plugin = load_plugin(\"./libscallion.so\");\n\
}\n\
\n\
on(time 0:02){\n\
...
\n\
\tcpu_cdf = load_cdf(\"" + args.cdfs + "cpu_speed_Bps.cdf\");")

        self.out("}\n\non(time 0:03){")
        # get all latency cdfs
        for region1 in self.cp.sg.shadowregions:
            for region2 in self.cp.sg.shadowregions:
                self.out("\t" + region1.name + "_to_" + region2.name + "_latency_cdf = load_cdf(\"" + args.cdfs + region1.name + "_to_" + region2.name + ".cdf\");")
...
\t\t\"dirauth 102400 " + args.authrc + " " + args.data + "authoritydata " + args.geoip + "\");\n")

        # spread out the servers geographically
        networks = ["US_WEST_network", "US_CENTRAL_network", "US_EAST_network", "EU_WEST_network"]
        for network in networks:
            self.out("\tcreate_nodes(" + str(args.nfs/len(networks)) + ", fs_plugin, " + network + ", fs_hostname, fastbw_cdf, NULL, cpu_cdf, \"8080 www\");")

        remain = args.nfs%len(networks)
        if remain > 0:
            self.out("\tcreate_nodes(" + str(remain) + ", fs_plugin, US_CENTRAL_network, fs_hostname, fastbw_cdf, NULL, cpu_cdf, \"8080 www\");\n")
        ...
        for i in range(args.nfs):
            if i == 0: print >> f, "fileserver.shd:8080:/5mb.urnd"
            else: print >> f, str(i) + ".fileserver.shd:8080:/5mb.urnd"
        f.close()
        f = open(args.run + "web.spec", 'w')
        for i in range(args.nfs):
            if i == 0: print >> f, "fileserver.shd:8080:/320kb.urnd"
            else: print >> f, str(i) + ".fileserver.shd:8080:/320kb.urnd"
        f.close()
...
\t\t\"exitrelay " + str(exit.torbw) + " " + args.exitrc + " " + args.data + "exitdata " + args.geoip + "\");")
                self.out("}\n")
...
\t\t\"relay " + str(relay.torbw) + " " + args.relayrc + " " + args.data + "relaydata " + args.geoip + "\");")
                self.out("}\n")
...
\t\t\"relay " + str(relay.torbw) + " " + args.relayrc + " " + args.data + "relaydata " + args.geoip + "\");")
        ...
        WEB=" multi " + args.run + "web.spec localhost 9000 " + args.cdfs + "http_think_times.cdf -1"
...
\t\t\"client 102400 " + args.clientrc + " " + args.data + "clientdata " + args.geoip + WEB + "\");\n");
...
\t\t\"client 102400 " + args.clientrc + " " + args.data + "clientdata " + args.geoip + BULK + "\");")
