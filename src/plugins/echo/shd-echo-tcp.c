...
static EchoClient* _echotcp_newClient(ShadowLogFunc log, in_addr_t serverIPAddress) {
	...
	if (result == -1 && errno != EINPROGRESS) {
		log(SHADOW_LOG_LEVEL_WARNING, __FUNCTION__, "Error in connect");
		return NULL;
	}
...
static EchoServer* _echotcp_newServer(ShadowLogFunc log, in_addr_t bindIPAddress) {
	...
	if (result == -1) {
		log(SHADOW_LOG_LEVEL_WARNING, __FUNCTION__, "error in listen");
		return NULL;
	}
...
static gboolean _echotcp_newPair(ShadowLogFunc log, EchoClient** client, EchoServer** server) {
	...
	if(result == -1) {
		log(SHADOW_LOG_LEVEL_WARNING, __FUNCTION__, "Error in socketpair");
		return FALSE;
	}
...
static void _echotcp_serverReadable(EchoServer* es, gint socketd) {
	...
	if(socketd == es->listend) {
		...
		if((acceptedDescriptor = accept(es->listend, NULL, NULL)) == -1) {
			es->log(SHADOW_LOG_LEVEL_WARNING, __FUNCTION__, "error accepting socket");
			return;
		}
		struct epoll_event ev;
		ev.events = EPOLLIN;
...
static void _echotcp_serverWritable(EchoServer* es, gint socketd) {
	es->log(SHADOW_LOG_LEVEL_DEBUG, __FUNCTION__, "trying to write socket %i", socketd);
...
void echotcp_ready(EchoTCP* etcp) {
	...
	if(etcp->client) {
		...
		if(nfds == -1) {
			etcp->log(SHADOW_LOG_LEVEL_WARNING, __FUNCTION__, "error in epoll_wait");
		}
