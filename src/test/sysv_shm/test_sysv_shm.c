...
static int run_owner_perms(key_t key) {
    ...
    if (geteuid() == 0) {
        // Root can bypass SysV shm mode bits only with the relevant capability,
        // which containerized CI may not provide. Non-root still expects EACCES.
        if (p == (void*)-1 && errno != EACCES) {
            shmctl(shmid, IPC_RMID, NULL);
            return fail_errno("shmat(owner-perms root)");
        }
        if (p != (void*)-1 && shmdt(p) < 0) {
            shmctl(shmid, IPC_RMID, NULL);
            return fail_errno("shmdt(owner-perms root)");
        }
    } else {
        if (p != (void*)-1) {
            shmdt(p);
            shmctl(shmid, IPC_RMID, NULL);
            return fail_msg("shmat(owner-perms) unexpectedly succeeded");
        }
        if (errno != EACCES) {
            shmctl(shmid, IPC_RMID, NULL);
