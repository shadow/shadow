...
static EchoClient* _echotcp_newClient(LogFunc log, in_addr_t serverIPAddress) {
    g_assert(log);
...
static EchoServer* _echotcp_newServer(LogFunc log, in_addr_t bindIPAddress) {
    g_assert(log);
...
static gboolean _echotcp_newPair(LogFunc log, EchoClient** client, EchoServer** server) {
...
EchoTCP* echotcp_new(LogFunc log, int argc, char* argv[]) {
    g_assert(log);
    ...
    if (g_ascii_strncasecmp(mode, "client", 6) == 0) {
        if (argc < 2) {
            ...
            if (getaddrinfo(serverHostName, NULL, NULL, &serverInfo) == -1) {
                log(G_LOG_LEVEL_WARNING, __FUNCTION__,
                        "unable to create client: error in getaddrinfo");
                isError = TRUE;
            } else {
        ...
        if (result == 0) {
            ...
            if (getaddrinfo(myHostName, NULL, NULL, &myInfo) == -1) {
                log(G_LOG_LEVEL_WARNING, __FUNCTION__,
                        "unable to create server: error in getaddrinfo");
                isError = TRUE;
            } else {
            ...
            log(G_LOG_LEVEL_WARNING, __FUNCTION__,
                    "unable to create server: error in gethostname");
            isError = TRUE;
        }
        ...
        etcp->client = _echotcp_newClient(log, serverIP);
        if(!etcp->server || !etcp->client) {
            isError = TRUE;
        }
    } else if (g_ascii_strncasecmp(mode, "socketpair", 10) == 0) {
        if(!_echotcp_newPair(log, &(etcp->client), &(etcp->server))) {
            isError = TRUE;
        }
    } else {
        isError = TRUE;
...
static void _echotcp_clientReadable(EchoClient* ec, gint socketd) {
    ...
    if (!ec->is_done) {
        ...
        while (ec->amount_sent - ec->recv_offset > 0
                && (b = recv(socketd, ec->recvBuffer + ec->recv_offset,
                        ec->amount_sent - ec->recv_offset, 0)) > 0) {
            ec->log(G_LOG_LEVEL_DEBUG, __FUNCTION__, "client socket %i read %i bytes: '%s'",
                    socketd, b, ec->recvBuffer + ec->recv_offset);
            ec->recv_offset += b;
        }
        ...
        if (ec->recv_offset >= ec->amount_sent) {
            ...
            ec->log(G_LOG_LEVEL_INFO, __FUNCTION__, "echo progress: %i of %i bytes",
    ...
    if (socketd == es->listend) {
        ...
        if (read_size > 0) {
            ...
            if (bread == 0) {
                ...
                es->log(G_LOG_LEVEL_INFO, __FUNCTION__, "server socket %i read %i bytes",
                        socketd, (gint) bread);
...
static void _echotcp_clientWritable(EchoClient* ec, gint socketd) {
    if (!ec->sent_msg) {
        ...
        ssize_t b = send(socketd, ec->sendBuffer + ec->amount_sent,
                sizeof(ec->sendBuffer) - ec->amount_sent, 0);
        ec->amount_sent += b;
        ec->log(G_LOG_LEVEL_DEBUG, __FUNCTION__, "client socket %i wrote %i bytes: '%s'",
                socketd, b, ec->sendBuffer);
...
static void _echotcp_serverWritable(EchoServer* es, gint socketd) {
    ...
    if (write_size > 0) {
        ...
        if (bwrote == 0) {
            ...
            es->log(G_LOG_LEVEL_INFO, __FUNCTION__, "server socket %i wrote %i bytes", socketd,
                    (gint) bwrote);
...
int echotcp_ready(EchoTCP* etcp) {
    g_assert(etcp);
