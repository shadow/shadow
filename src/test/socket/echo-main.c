...
gint main(gint argc, gchar *argv[]) {
    ...
    const char* USAGE =
            "Echo USAGE: 'tcp client serverIP', 'tcp server', 'tcp loopback', 'tcp socketpair', "
    ...
    EchoServer* server = echostate.etcp ? echostate.etcp->server :
                         echostate.eudp ? echostate.eudp->server : NULL;
    EchoClient* client = echostate.etcp ? echostate.etcp->client :
                         echostate.eudp ? echostate.eudp->client : NULL;
    ...
    while (1) {
        ...
        if(client && server) {
            if(client->is_done && server->is_done) {
                if(client->has_error || server->has_error) {
                    isError = TRUE;
                } else {
                    isError = FALSE;
                }
