...
static EchoClient* _echoudp_newClient(LogFunc log, in_addr_t serverIPAddress) {
    g_assert(log);
...
static EchoServer* _echoudp_newServer(LogFunc log, in_addr_t bindIPAddress) {
    g_assert(log);
...
EchoUDP* echoudp_new(LogFunc log, int argc, char* argv[]) {
    g_assert(log);
...
static void _echoudp_clientReadable(EchoClient* ec, gint socketd) {
    ...
    if (!ec->is_done) {
        ...
        while (ec->amount_sent - ec->recv_offset > 0
                && (b = recvfrom(socketd, ec->recvBuffer + ec->recv_offset,
                        ec->amount_sent - ec->recv_offset, 0, NULL, NULL)) > 0) {
...
static void _echoudp_serverReadable(EchoServer* es, gint socketd) {
    ...
    if (read_size > 0) {
        ssize_t bread = recvfrom(socketd, es->echoBuffer + es->read_offset, read_size, 0,
                (struct sockaddr*) &es->address, &len);
        ...
        if (bread == 0) {
            ...
            es->log(G_LOG_LEVEL_INFO, __FUNCTION__, "server socket %i read %i bytes", socketd,
                    (gint) bread);
...
static void _echoudp_clientWritable(EchoClient* ec, gint socketd) {
    if (!ec->sent_msg) {
        ...
        ssize_t b = sendto(socketd, ec->sendBuffer, sizeof(ec->sendBuffer), 0,
...
static void _echoudp_serverWritable(EchoServer* es, gint socketd) {
    ...
    if (write_size > 0) {
        ssize_t bwrote = sendto(socketd, es->echoBuffer + es->write_offset, write_size, 0,
                (struct sockaddr*) &es->address, len);
        if (bwrote == 0) {
