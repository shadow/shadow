...
typedef enum _TGenActionType TGenActionType;
enum _TGenActionType {
	TGEN_ACTION_START, TGEN_ACTION_END, TGEN_ACTION_PAUSE,
	TGEN_ACTION_TRANSFER, TGEN_ACTION_SYNCHRNOIZE,
};

// XXX this can be made more memory efficient
struct _TGenAction {
	TGenActionType type;
	gpointer key;
	gboolean hasKey;
...
static GError* _tgengraph_handlePeer(const gchar* attributeName, const gchar* peerStr,
		TGenPeer* peer) {
	...
	if(!tokens[0] || !tokens[1]) {
		error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
						"expected peer syntax 'hostname:port' for attribute '%s'", attributeName);
		return error;
	}

	/* dont add my own address to the server pool */
	char myname[128];
	if(!gethostname(&myname[0], 128) && !g_ascii_strcasecmp(myname, tokens[0])) {
	...
	if(address == INADDR_ANY || address == INADDR_NONE) {
		error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
						...
						"expected 'localhost', '127.0.0.1', or valid node hostname", peerStr, attributeName);
	}
	if(portNum > UINT16_MAX) {
		error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
						"invalid peer '%s' for port part of attribute '%s', "
						"expected 16 bit unsigned integer", peerStr, attributeName);
	} else {
	...
	if(!error) {
		...
		const gchar* ipString = inet_ntop(AF_INET, &address, ipStringBuffer, INET6_ADDRSTRLEN);
...
static GError* _tgengraph_handlePeers(const gchar* attributeName, const gchar* peersStr,
		TGenPool* peerPool) {
...
static GError* _tgengraph_handleBytes(const gchar* attributeName, const gchar* byteStr,
		guint64* bytesOut) {
	...
	for(glong i = 0; i < bytesTokenLength; i++) {
		...
		if(!g_ascii_isdigit(c)) {
			error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
							...
							"expected format like '10240' or '10 KiB'", c, byteStr, attributeName);
			break;
		}
	}
	...
	if(!error) {
		...
		if(suffixToken) {
			...
			if(!error && base && exponent) {
				bytes = (guint64) (bytes * pow((gdouble)base, (gdouble)exponent));
			}
		}
...
static GError* _tgengraph_handleBoolean(const gchar* attributeName, const gchar* booleanStr,
		gboolean* booleanOut, gboolean* isFoundOut) {
	...
	if(!g_ascii_strcasecmp(booleanStr, "true") || !g_ascii_strcasecmp(booleanStr, "1")) {
		isFound= TRUE;
		boolean = TRUE;
	} else if (!g_ascii_strcasecmp(booleanStr, "false") || !g_ascii_strcasecmp(booleanStr, "0")) {
		isFound= TRUE;
		boolean = FALSE;
	} else {
		error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT,
						"invalid content for attribute '%s', "
						"expected boolean value 'true' or 'false'", attributeName);
	}
...
void tgenaction_free(TGenAction* action) {
	...
	if(action->start.peers) {
		tgenpool_free(action->start.peers);
	}
	if(action->transfer.peers) {
		tgenpool_free(action->transfer.peers);
	}
...
TGenAction* tgenaction_newStartAction(const gchar* timeStr, const gchar* serverPortStr,
	...
	*error = _tgengraph_handlePeers("peers", peersStr, peerPool);
	...
	action->start.time = g_ascii_strtoull(timeStr, NULL, 10);
	action->start.serverport = g_ascii_strtoull(serverPortStr, NULL, 10);
	action->start.peers = peerPool;
	action->start.socksproxy = socksproxy;
...
TGenAction* tgenaction_newEndAction(const gchar* timeStr, const gchar* countStr,
	...
	action->end.size = size;
	if(g_ascii_strncasecmp(timeStr, "\0", (gsize)1)) {
		action->end.time = g_ascii_strtoull(timeStr, NULL, 10);
	}
	if(g_ascii_strncasecmp(countStr, "\0", (gsize)1)) {
		action->end.count = g_ascii_strtoull(countStr, NULL, 10);
	}
...
TGenAction* tgenaction_newPauseAction(const gchar* timeStr, GError** error) {
	...
	action->pause.time = g_ascii_strtoull(timeStr, NULL, 10);
...
TGenAction* tgenaction_newSynchronizeAction(GError** error) {
	...
	action->type = TGEN_ACTION_SYNCHRNOIZE;
...
TGenAction* tgenaction_newTransferAction(const gchar* typeStr, const gchar* protocolStr,
	...
	if(!g_ascii_strncasecmp(typeStr, "\0", (gsize)1)) {
		...
		*error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ATTRIBUTE,
				"transfer action has unknown value '%s' for 'type' attribute", typeStr);
		return NULL;
	}
	...
	if(!g_ascii_strncasecmp(protocolStr, "\0", (gsize)1)) {
		...
		*error = g_error_new(G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ATTRIBUTE,
				"transfer action has unknown value '%s' for 'protocol' attribute", protocolStr);
		return NULL;
	}
	...
	*error = _tgengraph_handleBytes("size", sizeStr, &size);
	...
	action->transfer.protocol = protocol;
	action->transfer.type = type;
	action->transfer.size = size;
	action->transfer.peers = peerPool;
...
guint64 tgenaction_getServerPort(TGenAction* action) {
	TGEN_ASSERT(action);
	g_assert(action->type == TGEN_ACTION_START);
	return action->start.serverport;
}
