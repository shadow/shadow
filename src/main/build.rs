...
fn run_cbindgen(build_common: &ShadowBuildCommon) {
    ...
    // bindings.h:
    {
        ...
        // so we need to include those here. We can't use 'includes', because
        // it's overriden by "no_includes" in the base config.
        config.add_includes_as_raw(&[
            "lib/logger/log_level.h",
            "lib/shadow-shim-helper-rs/shim_helper.h",
            "lib/shmem/shmem_allocator.h",
            "lib/tsc/tsc.h",
            "main/bindings/c/bindings-opaque.h",
            "main/core/worker.h",
            "main/host/descriptor/descriptor_types.h",
            "main/host/descriptor/tcp.h",
            "main/host/futex_table.h",
            "main/host/network_interface.h",
            "main/host/protocol.h",
            "main/host/status_listener.h",
            "main/host/syscall_handler.h",
            "main/host/syscall_types.h",
            "main/host/tracker_types.h",
            "main/routing/dns.h",
            "main/routing/packet.minimal.h",
        ]);
        config.add_sys_includes_as_raw(&["sys/socket.h", "netinet/in.h", "arpa/inet.h"]);
        // We have to manually create the vararg declaration.  See
        // crate::main::host::thread::export::thread_nativeSyscall.
        config
            .add_after_includes("long thread_nativeSyscall(const Thread* thread, long n, ...);\n");
    ...
    // bindings-opaque.h
    {
        ...
        config.add_after_includes("typedef struct _SysCallCondition SysCallCondition;\n");
        cbindgen::Builder::new()
...
fn main() {
    ...
    //FIXME run_bindgen(&build_common);
