...
use linux_api::posix_types::{
    kernel_gid32_t, kernel_key_t, kernel_pid_t, kernel_size_t, kernel_uid32_t,
};
use linux_api::shm::{ShmatFlags, ShmgetFlags, ShmgetFlagsParts, shmid64_ds};
...
impl Segment {
    fn new(
        ...
        key: kernel_key_t,
        size: usize,
        shmflg: ShmgetFlagsParts,
        creator_pid: ProcessId,
    ) -> Self {
        let creator_uid: kernel_uid32_t = nix::unistd::geteuid().as_raw();
        let creator_gid: kernel_gid32_t = nix::unistd::getegid().as_raw();
        let mode = shmflg.perms.bits().try_into().unwrap();
    ...
    fn has_permission(&self, permissions: SFlag) -> bool {
        let euid: kernel_uid32_t = nix::unistd::geteuid().as_raw();
        let egid: kernel_gid32_t = nix::unistd::getegid().as_raw();
        ...
        let requested_mode =
            (permissions.bits() | (permissions.bits() >> 3) | (permissions.bits() >> 6)) as u16;
        let granted_mode = if euid == self.uid {
            self.mode >> 6
        ...
        (requested_mode & !granted_mode & 0o7) == 0
    }

    fn can_attach(&self, read_only: bool) -> bool {
        let mut permissions = SFlag::S_IRUSR | SFlag::S_IRGRP | SFlag::S_IROTH;
        if !read_only {
            permissions |= SFlag::S_IWUSR | SFlag::S_IWGRP | SFlag::S_IWOTH;
        }

        self.has_permission(permissions)
    }
    ...
    fn to_shmid_ds(&self) -> shmid64_ds {
        let mut ds: shmid64_ds = unsafe { std::mem::zeroed() };
        ds.shm_perm.key = self.key;
        ...
        ds.shm_perm.seq = 0;
        ds.shm_segsz = self.size.try_into().unwrap_or(kernel_size_t::MAX);
...
pub struct SysvShmNamespace {
    ...
    segments_by_key: HashMap<kernel_key_t, i32>,
...
impl SysvShmNamespace {
    ...
    pub fn shmget(
        ...
        let shmflg = ShmgetFlagsParts::from_shmflg(shmflg);
        let create = shmflg.flags.contains(ShmgetFlags::IPC_CREAT);
        let exclusive = shmflg.flags.contains(ShmgetFlags::IPC_EXCL);
        ...
        if let Some(shmid) = self.segments_by_key.get(&key).copied() {
            ...
            if !segment.has_permission(shmflg.perms) {
    ...
    pub fn shmat(
        ...
        let read_only = ShmatFlags::from_bits_retain(shmflg).contains(ShmatFlags::SHM_RDONLY);
    ...
    pub fn shmctl_ipc_stat(&self, shmid: i32) -> Result<shmid64_ds, Errno> {
        let segment = self.segments_by_id.get(&shmid).ok_or(Errno::EINVAL)?;
