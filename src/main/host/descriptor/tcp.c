        ...
        case TCPS_CLOSED: {
            ...
            if(tcp->child && tcp->child->parent) {
                ...
                if((parent->state == TCPS_CLOSED) && (g_hash_table_size(parent->server->children) <= 0)) {
                    /* Previously accepted sockets were still children of the
                     * listening socket.  This is no longer the case, so we
                     * shouldn't be able to get here.  child sockets are
                     * unparented when they are accepted from the listening
                     * queue, and when a listening socket is closed, we close all
                     * child sockets (i.e. sockets still on the listen queue).
                     */
