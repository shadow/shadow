...
static Packet* _tcp_createDataPacket(TCP* tcp, ThreadRc* thread, enum ProtocolTCPFlags flags,
...
/* Address and port must be in network byte order. */
static gssize _tcp_sendUserData(LegacySocket* socket, ThreadRc* thread, PluginVirtualPtr buffer,
...
/* Address and port must be in network byte order. */
static gssize _tcp_receiveUserData(LegacySocket* socket, ThreadRc* thread, PluginVirtualPtr buffer,
                                   gsize nBytes, in_addr_t* ip, in_port_t* port) {
    TCP* tcp = _tcp_fromLegacyFile((LegacyFile*)socket);
    MAGIC_ASSERT(tcp);
