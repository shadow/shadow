...
static gint _descriptortable_compareInts(gconstpointer a, gconstpointer b, gpointer data) {
...
static void _descriptortable_trimIndiciesTail(DescriptorTable* table) {
    MAGIC_ASSERT(table);
    while(GPOINTER_TO_INT(g_queue_peek_tail(table->availableIndices)) == table->indexCounter) {
...
bool descriptortable_remove(DescriptorTable* table, Descriptor* descriptor) {
    ...
    if(g_hash_table_contains(table->descriptors, indexp)) {
        ...
        g_queue_insert_sorted(table->availableIndices, indexp, _descriptortable_compareInts, NULL);
...
void descriptortable_setStdOut(DescriptorTable* table, Descriptor* descriptor) {
    MAGIC_ASSERT(table);
    g_hash_table_insert(table->descriptors, GINT_TO_POINTER(STDOUT_FILENO), descriptor);
    descriptor_setHandle(descriptor, STDOUT_FILENO);
}

void descriptortable_setStdErr(DescriptorTable* table, Descriptor* descriptor) {
    MAGIC_ASSERT(table);
    g_hash_table_insert(table->descriptors, GINT_TO_POINTER(STDERR_FILENO), descriptor);
