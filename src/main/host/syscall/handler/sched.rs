...
impl SyscallHandler {
    ...
    pub fn sched_getaffinity(
        ...
        if cpusetsize < KERNEL_CPUSETSIZE_BYTES {
            // sched_getaffinity(2): EINVAL (sched_getaffinity() and, in kernels
            // before 2.6.9, sched_setaffinity()) cpusetsize is smaller than the
    ...
    pub fn sched_setaffinity(
        ...
        let to_copy = std::cmp::min(mask.len(), mask_ptr.len());
        ctx.objs
            .process
            .memory_borrow()
            .copy_from_ptr(&mut mask[..to_copy], mask_ptr.slice(..to_copy))?;
