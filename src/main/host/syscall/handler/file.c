...
SyscallReturn syscallhandler_fstatfs(SyscallHandler* sys, const SyscallArgs* args) {
    ...
    struct statfs buf = {0};
    /* Read the current value of the struct; the syscall can leave some fields
     * unmodified, so we need the initial values. */
    errcode = process_readPtr(rustsyscallhandler_getProcess(sys), &buf, bufPtr, sizeof(buf));
    if (errcode < 0) {
        return syscallreturn_makeDoneErrno(-errcode);
    }
