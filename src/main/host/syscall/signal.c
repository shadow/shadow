...
static SyscallReturn _syscallhandler_signalThread(SysCallHandler* sys, const Thread* thread,
    ...
    if (action.u.ksa_handler == SIG_IGN ||
        (action.u.ksa_handler == SIG_DFL && linux_defaultAction(sig) == LINUX_DEFAULT_ACTION_IGN)) {
...
static SyscallReturn _rt_sigaction(SysCallHandler* sys, int signum, UntypedForeignPtr actPtr,
    ...
    if (oldActPtr.val) {
        ...
        int rv = process_writePtr(
            _syscallhandler_getProcess(sys), oldActPtr, &old_action, sizeof(old_action));
        if (rv != 0) {
...
SyscallReturn syscallhandler_sigaltstack(SysCallHandler* sys, const SysCallArgs* args) {
    ...
    if (ss_ptr.val) {
        ...
        shimshmem_setSigAltStack(host_getShimShmemLock(_syscallhandler_getHost(sys)),
                                 thread_sharedMem(_syscallhandler_getThread(sys)), new_ss);
    }

    if (old_ss_ptr.val) {
        int rv =
            process_writePtr(_syscallhandler_getProcess(sys), old_ss_ptr, &old_ss, sizeof(old_ss));
        if (rv != 0) {
            return syscallreturn_makeDoneErrno(-rv);
        }
    }
...
static SyscallReturn _rt_sigprocmask(SysCallHandler* sys, int how, UntypedForeignPtr setPtr,
    ...
    if (oldSetPtr.val) {
        int rv = process_writePtr(
            _syscallhandler_getProcess(sys), oldSetPtr, &current_set, sizeof(current_set));
        if (rv < 0) {
            return syscallreturn_makeDoneErrno(-rv);
        }
    }
    ...
    if (setPtr.val) {
        ...
        shimshmem_setBlockedSignals(host_getShimShmemLock(_syscallhandler_getHost(sys)),
                                    thread_sharedMem(_syscallhandler_getThread(sys)), current_set);
    }
