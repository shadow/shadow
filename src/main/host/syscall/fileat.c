...
static int _syscallhandler_validateDirHelper(SysCallHandler* sys, int dirfd,
    ...
    Descriptor* desc = host_lookupDescriptor(sys->host, dirfd);
...
static int _syscallhandler_validateDirAndPathnameHelper(SysCallHandler* sys, int dirfd, PluginPtr pathnamePtr, File** dir_desc_out, const char** pathname_out) {
    ...
    errcode = thread_getReadableString(sys->thread, pathnamePtr, PATH_MAX, pathname_out, NULL);
    if (errcode < 0) {
        return errcode;
    }
...
static SysCallReturn _syscallhandler_renameatHelper(SysCallHandler* sys, int olddirfd, PluginPtr oldpathPtr, int newdirfd, PluginPtr newpathPtr, unsigned int flags) {
    /* Validate params. */
    File* olddir_desc = NULL;
    const char* oldpath;

    int errcode = _syscallhandler_validateDirAndPathnameHelper(sys, olddirfd, oldpathPtr, &olddir_desc, &oldpath);
    if (errcode < 0) {
    ...
    errcode = _syscallhandler_validateDirAndPathnameHelper(sys, newdirfd, newpathPtr, &newdir_desc, &newpath);
    if (errcode < 0) {
        return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = errcode};
    }

    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_renameat2(olddir_desc, oldpath, newdir_desc, newpath, flags)};
}
...
SysCallReturn syscallhandler_openat(SysCallHandler* sys,
    ...
    debug("Trying to openat file with path name at plugin addr %p", pathnamePtr.val);
    ...
    Descriptor* file_desc = host_createDescriptor(sys->host, DT_FILE);
    utility_assert(file_desc);
    utility_assert(_syscallhandler_validateDescriptor(file_desc, DT_FILE) == 0);

    /* Now open the file. */
    errcode = file_openat((File*)file_desc, dir_desc, pathname, flags, mode);
    if (errcode < 0) {
        /* This will remove the descriptor entry and unref/free the File. */
        descriptor_close(file_desc);
    }
...
SysCallReturn syscallhandler_newfstatat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_fstatat(dir_desc, pathname, buf, flags)};
}
...
SysCallReturn syscallhandler_fchownat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_fchownat(dir_desc, pathname, owner, group, flags)};
}
...
SysCallReturn syscallhandler_fchmodat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_fchmodat(dir_desc, pathname, mode, flags)};
}
...
SysCallReturn syscallhandler_futimesat(SysCallHandler* sys,
    ...
    const struct timeval* times = thread_getReadablePtr(sys->thread, timesPtr, 2*sizeof(*times));

    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_futimesat(dir_desc, pathname, times)};
}
...
SysCallReturn syscallhandler_utimensat(SysCallHandler* sys,
    ...
    const struct timespec* times = thread_getReadablePtr(sys->thread, timesPtr, 2*sizeof(*times));

    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_utimensat(dir_desc, pathname, times, flags)};
}
...
SysCallReturn syscallhandler_faccessat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_faccessat(dir_desc, pathname, mode, flags)};
}
...
SysCallReturn syscallhandler_mkdirat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_mkdirat(dir_desc, pathname, mode)};
}
...
SysCallReturn syscallhandler_mknodat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_mknodat(dir_desc, pathname, mode, dev)};
}
...
SysCallReturn syscallhandler_linkat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_linkat(olddir_desc, oldpath, newdir_desc, newpath, flags)};
}
...
SysCallReturn syscallhandler_unlinkat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_unlinkat(dir_desc, pathname, flags)};
}
...
SysCallReturn syscallhandler_symlinkat(SysCallHandler* sys,
    ...
    int errcode = _syscallhandler_validateDirAndPathnameHelper(sys, dirfd, linkpathPtr, &dir_desc, &linkpath);
    if (errcode < 0) {
        return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = errcode};
    }
    ...
    errcode = thread_getReadableString(sys->thread, targetpathPtr, PATH_MAX, &targetpath, NULL);
    if (errcode < 0) {
        return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = errcode};
    }

    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_symlinkat(dir_desc, linkpath, targetpath)};
}
...
SysCallReturn syscallhandler_readlinkat(SysCallHandler* sys,
    ...
    return (SysCallReturn){.state = SYSCALL_DONE, .retval.as_i64 = file_readlinkat(dir_desc, pathname, buf, bufSize)};
}

SysCallReturn syscallhandler_renameat(SysCallHandler* sys,
                                          const SysCallArgs* args) {
    return _syscallhandler_renameatHelper(sys, args->args[0].as_i64, args->args[1].as_ptr, args->args[2].as_i64, args->args[3].as_ptr, 0);
}
