...
EXPORT void *
dlvsym (void *handle, const char *symbol, const char *version)
{
...
EXPORT void *
dlmopen (Lmid_t lmid, const char *filename, int flag)
{
  return vdl_dlmopen_public (lmid, filename, flag);
}

EXPORT Lmid_t
dl_lmid_new (int argc, char **argv, char **envp)
{
  return vdl_dl_lmid_new_public (argc, argv, envp);
}

EXPORT void
dl_lmid_delete (Lmid_t lmid)
{
  return vdl_dl_lmid_delete_public (lmid);
}

EXPORT int
dl_lmid_add_callback (Lmid_t lmid,
...
EXPORT int
dl_lmid_add_lib_remap (Lmid_t lmid, const char *src, const char *dst)
{
  return vdl_dl_lmid_add_lib_remap_public (lmid, src, dst);
}

EXPORT int
dl_lmid_add_symbol_remap (Lmid_t lmid,
                          const char *src_name,
