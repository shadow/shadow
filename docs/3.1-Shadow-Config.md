...
- [`host_defaults.type_hint`](#host_defaultstype_hint)
- [`hosts`](#hosts)
...
Default: "message"  
Type: "error" OR "critical" OR "warning" OR "message" OR "info" OR "debug" OR "trace"

Log level of output written on stdout. If Shadow was built in release mode, then log messages at a level lower than 'info' will always be dropped.
...
Default: null  
Type: "error" OR "critical" OR "warning" OR "message" OR "info" OR "debug" OR "trace" OR null
...
This hint will be used to assign the host to a node based on the types of nodes in the network topology.
