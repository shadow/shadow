...
* It may cause the simulation to run slower.
  ...
  * Even when the timer never fires, there is some additional overhead for the
    additional bookkeeping and management of the native timers. This overhead
    *usually* won't be very noticeable other than in managed code that makes
    syscalls in a very tight loop.
